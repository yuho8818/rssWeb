...
$(document).ready(function () {
    ...
    $("#button_sure").click(function () {
        var flag= true
        var SEWAGE_INFOUID = $("#SEWAGE_INFOUID").val();
        var dischargeArea = $("#dischargeArea").val();
        var dischargeDate = $("#dischargeDate").val();
        flag = isDate(dischargeDate)
        isDate(dischargeDate)?null:$("#dischargeDate").css("background-color","red")
        isDate(dischargeDate)?null:$("#dischargeDate").val("请输入正确的日期！")
    ...
    //关闭子表模态框
    $("#button_cancel_child").click(function () {
        $("#button_cancel_hidden_child").click()
    })

    /*子表增加*/
    $("#button-add_childtable").click(function () {
        alert("子表新增确认信息")
        $("#table_model_name_child").html("子表记录增加")
        ...
        $(tr).find("td").each(function () {
            ...
            switch ($(this).index()) {
                case 2:
                    $("#SEWAGE_INFOUID_main").val(x);
                    break;
                case 3:
                    $("#dischargeArea_main").val(x);
                    break;
            }
        })
        $("#table_row_no").attr("disabled", true);
        $("#table_model_child").modal();
    })
...
function prepare() {
    ...
    $.ajax({
        ...
        success: function (data) {
            $("#data_table").html($(data).find("#data_table").html());
            $("#is_page_count").html($(data).find("#is_page_count").html());
            $("#sewage_equip").html($(data).find("#equip_name").html());
        },
    });
